...
from MA.UI import keystroke
from MA.UI import mouseDrag
...
def test_drag_drop(app_dialog):
    # Open a Windows file explorer
    topwindows.buttons["Start"].get().mouseClick()
    time.sleep(1)
    keystroke("{TAB}")
    time.sleep(0.5)
    keystroke("{ENTER}")
    ...
    #topwindows.dialogs["Pictures"].listitems["achmed.JPG"].get().mouseClick()
    mouseDrag(topwindows.dialogs["Pictures"].listitems["achmed.JPG"], None, None, topwindows["Shotgun: Publish"])
    time.sleep(2)
